...
__global__ void layer_256_1(int step, half2 *in, half *F_real, half *F_imag)
{
    ...
    float2 twiddle_unit = W_N_K_fp32(step * 16, glb_col); // precision improved
    ...
    */
    ...
    for (int i_start = 0; i_start < 256 * CONT_SIZE; i_start += NUM_WARP * 256)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            ...
            // half2 ele = smem_in[eid]; // opt test
            ...
            // twiddle_factor = cmul(twiddle_factor, twiddle_unit);
            twiddle_factor = cuCmulf(twiddle_factor, twiddle_unit); // precision improved
        }
    ...
    */
    ...
    for (int i_start = 0; i_start < CONT_SIZE / NUM_WARP; i_start++)
    {
        ...
        float2 twiddle_unit_2 = W_N_K_fp32(step * 256, glb_col_2); // precision improved
        wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_real;
        wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_imag;
        wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::col_major> frag_in_real;
        wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::col_major> frag_in_imag;
...
__global__ void layer_256_1_A100(int step, half2 *in, half *F_real, half *F_imag)
{
    ...
    half2 twiddle_factor = {1.0, 0};
    half2 twiddle_unit = W_N_K(step * 16, blockIdx.x * CONT_SIZE + threadIdx.x);
    // for (int i = 0; i < 256 * CONT_SIZE; i += NUM_WARP * 32)
    // {
    //     int eid = i + t_block;
    //     smem_in[eid] = in[block_start + eid / CONT_SIZE * step + eid % CONT_SIZE];
...
__global__ void layer_512_0(half2 *in, half *F_real, half *F_imag)
{
    ...
    */
    ...
    for (int i = 0; i < 512 * CONT_SIZE; i += NUM_WARP * 16 * 16)
    {
        ...
        wmma::load_matrix_sync(frag_in_tmp0, (half *)(in + block_start + warp_start), 32);
        wmma::load_matrix_sync(frag_in_tmp1, (half *)(in + block_start + warp_start) + 16, 32);
        // wmma::load_matrix_sync(frag_in_tmp0, (half *)(smem_in + warp_start), 32); //opt test
        // wmma::load_matrix_sync(frag_in_tmp1, (half *)(smem_in + warp_start) + 16, 32); //opt test
...
__global__ void layer_512_1(int step, half2 *in, half *F_real, half *F_imag)
{
    ...
    float2 twiddle_unit_2 = W_N_K_fp32(step * 512, 256 / CONT_SIZE * step); // precision improved
    // half2 twiddle_factor = W_N_K(step * 512, blockIdx.x * CONT_SIZE + t_block / CONT_SIZE * step + t_block % CONT_SIZE);
    float2 twiddle_factor = W_N_K_fp32(step * 512, blockIdx.x * CONT_SIZE + t_block / CONT_SIZE * step + t_block % CONT_SIZE); // precision improved
    for (int i = 0; i < 256 * CONT_SIZE; i += NUM_WARP * 32)
    {
        int eid = i + t_block;
        half2 ele_0 = smem_in[eid];
...
__global__ void layer_512_1_A100(int step, half2 *in, half *F_real, half *F_imag)
{
    ...
    half2 twiddle_factor = {1.0, 0};
    half2 twiddle_unit = W_N_K(step * 16, blockIdx.x * CONT_SIZE + threadIdx.x % 16);
...
void tcfftExec(tcfftHandle plan, half *data)
{
    ...
    plan.layer_0[plan.mergings[0]]<<<plan.N * plan.N_batch / n_cont[plan.mergings[0]] / RADIX, threads, RADIX * sizeof(half2) * n_cont[plan.mergings[0]]>>>((half2 *)data, plan.F_real, plan.F_imag);
    step *= RADIX;
    ...
    for (int i = 1; i < plan.n_mergings; ++i)
    {
        ...
        cudaFuncSetAttribute(plan.layer_1[plan.mergings[i]], cudaFuncAttributeMaxDynamicSharedMemorySize, RADIX * sizeof(half2) * n_cont[plan.mergings[i]]);
        plan.layer_1[plan.mergings[i]]<<<blocks, threads, RADIX * sizeof(half2) * n_cont[plan.mergings[i]]>>>(step, (half2 *)data, plan.F_real, plan.F_imag);
        step *= RADIX;
    }
}
...
void finalize(double *result,int N_batch)
{
